...
INSTRUCTION_FOR_ACKNOWLEDGEMENT = "Based on the user's last message, provide a brief, one-sentence acknowledgment to show you've understood. For example, if the user provides their details for an inquiry, you might say 'Thanks for sharing that.' or if they ask a question, 'That's a great question.' Your acknowledgement MUST be a single sentence and MUST NOT contain any questions or ask for information."
INSTRUCTION_ASK_USER_DATA = "Acknowledge the user's last message and confirm you can help. Then, politely ask for their name and email address before continuing. For example: 'I'll be glad to assist you by providing more information. Before we continue, who do I have the pleasure of speaking with? Could you also share your email address?'"
