...
PROMPT_ANSWER_ABOUT_CONDITION = "You have confirmed that the user's condition is treated based on the provided context. Now, respond to the user by: 1. Confirming we treat the condition. 2. Briefly explaining our approach based on the relevant category from the context. Do not include a greeting. IMPORTANT: Do NOT ask for any personal details, age, symptoms, or medical history. Simply provide the information and wait for their next question."
PROMPT_FAREWELL_MESSAGE="It was a pleasure to assist you, see you the next time!"
PROMPT_RECOMMEND_DOCTOR="Based on the user's condition and the doctor recommendation provided in the context, generate a friendly message recommending the doctor. For example: 'Dr. Silva would be a great fit as she specializes in immune concerns like psoriasis.' Do not include a greeting, and do not include the internal reasoning from the tool, just the recommendation."
